...
#define FLEXCOLOR_CF_BANDING            (1 << 3)
...
struct DRIVER_CONTEXT {
  ...
  // Common
  //
  void (* pfGetRect      )(DRIVER_CONTEXT * _pContext, GUI_RECT * pRect);
  void (* pfSendDirtyRect)(DRIVER_CONTEXT * _pContext);
  void (* pfReadTile     )(DRIVER_CONTEXT * _pContext, GUI_RECT * pRect);
  //
  // Controller dependent function pointers
  //
  void (* pfReadRect)(void);
  void (* pfDeleteHook)(DRIVER_CONTEXT * _pContext);
  //
  // Orientation
  //
  ...
  int        RowIndex;
  U32        BufferSize;
  GUI_RECT * pBandingRect;
  GUI_RECT   BandingRect;
  int        xSizeDriver;
  int        ySizeDriver;
  void     * pReadBuffer;
};
...
int  GUIDRV_FlexColor__Banding     (GUI_DEVICE * pDevice, U8 Cmd, U32 Para, void * pVoid);
